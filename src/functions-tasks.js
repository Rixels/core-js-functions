...
function getPolynom(...values) {
  ...
  return (y) => {
    return values.reduce((acc, item, index) => {
      return acc + item * y ** (values.length - index - 1);
    }, 0);
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const newArr = args.map((item) => JSON.stringify(item)).join(',');
    const newValue = `${func.name}(${newArr})`;
    logFunc(`${newValue} starts`);
    const result = func(...args);
    logFunc(`${newValue} ends`);
    return result;
  };
}
